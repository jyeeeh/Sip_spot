...
- **CONNECT**: Authorization 헤더 없으면 익명 연결 허용. 헤더가 있으면 반드시 유효해야 함
- **구독 권한**: `/topic/rooms/{code}` 구독은 인증 여부 무관 허용. 다른 destination은 거부
- **Principal 사용**: `accountId`는 반드시 `Principal`(`AccountPrincipal`)에서만 추출. 페이로드 값 신뢰 금지
